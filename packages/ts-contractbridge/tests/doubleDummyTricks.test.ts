...
describe('DoubleDummyTricks', () => {
  it('creates an empty instance', () => {
    const dd = DoubleDummyTricks.make()
    expect(dd.N).toEqual({})
    expect(dd.E).toEqual({})
    expect(dd.S).toEqual({})
    expect(dd.W).toEqual({})
  ...
  it('withTricks sets a single value without mutating the original', () => {
    const dd = DoubleDummyTricks.make()
    const updated = DoubleDummyTricks.withTricks(dd, 'N', 'NT', 9)
    expect(updated.N.NT).toBe(9)
    expect(dd.N.NT).toBeUndefined()
  })

  it('parses a PBN double dummy tricks string', () => {
    const dd = DoubleDummyTricks.fromPBN(SAMPLE_PBN)!
  ...
  it('round-trips through PBN', () => {
    const dd = DoubleDummyTricks.fromPBN(SAMPLE_PBN)!
    expect(DoubleDummyTricks.toPBN(dd)).toBe(SAMPLE_PBN)
  })

  it('treats F digits as unknown', () => {
    const pbn = 'F'.repeat(20)
    const dd = DoubleDummyTricks.fromPBN(pbn)!
    expect(dd.N).toEqual({})
    expect(DoubleDummyTricks.toPBN(dd)).toBe(pbn)
  })

  it('rejects a string of the wrong length', () => {
    expect(DoubleDummyTricks.fromPBN('7966879668')).toBeUndefined()
    expect(DoubleDummyTricks.fromPBN(SAMPLE_PBN + '7')).toBeUndefined()
  })
  ...
  it('rotates: N,S,E,W tricks move with the rotated direction', () => {
    const dd = DoubleDummyTricks.fromPBN(SAMPLE_PBN)!
    const rotated = DoubleDummyTricks.rotated(dd, 1)
  ...
  it('rotating by 0 seats is a no-op', () => {
    const dd = DoubleDummyTricks.fromPBN(SAMPLE_PBN)!
    expect(DoubleDummyTricks.rotated(dd, 0)).toEqual(dd)
  })
})
