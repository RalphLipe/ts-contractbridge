/** The parsed halves of a PBN tag line: [TagName "TagValue"]. */
...
export function parseTagLine(s: string): ParsedTag | undefined {
  const line = s.trim()
...
/** Formats a ParsedTag back into a PBN tag line: [TagName "TagValue"]. No trailing newline —
 *  joining lines together is left to the caller. Round-trips with parseTagLine. */
export function formatTagLine(tag: ParsedTag): string {
