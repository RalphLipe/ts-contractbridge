...
const make = (contract: Contract, declarer: Direction): DeclaredContract =>
  ({ contract, declarer })
...
const fromPBN = (s: string): DeclaredContract | undefined => {
...
const rotated = (dc: DeclaredContract, seats: number): DeclaredContract =>
  ({ contract: dc.contract, declarer: Direction.rotated(dc.declarer, seats) })

/** Returns negative if a < b, positive if a > b, 0 if equal. */
const compare = (a: DeclaredContract, b: DeclaredContract): number => {
